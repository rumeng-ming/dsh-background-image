window.__ModuleLoader__.load({
    ...
    `;
		...
		// ── pure helpers (extracted for unit testing via exports.__test) ──────
		function clampOpacity(strength) {
			return Math.max(0, Math.min(1, strength / 100));
		}

		function resolveSizeCss(size) {
			return size === "contain" ? "contain" : size === "cover" ? "cover" : String(size) + "%";
		}

		function buildBackgroundCss(light, dark, sizeCss, opacity) {
			return "body { background: #f4f5f8 !important; }\n" +
		...
		function applyBg(kind, light, dark, desc, strength, size, theme, imagePath) {
			...
			const opacity = clampOpacity(strength);
			const sizeCss = resolveSizeCss(size);
			const tag = document.createElement("style");
			tag.dataset.dyn = "dsh-background-image";
			tag.textContent = buildBackgroundCss(light, dark, sizeCss, opacity);
			document.head.appendChild(tag);
			cssDisposer = () => tag.remove();
			currentBg = { kind, rawLight: light, rawDark: dark, desc, strength, size, imagePath };
			saveBackground({
				kind,
				desc: desc ?? "",
				strength,
		...
		function BackgroundSettings(props) {
			...
			const [state, setState] = React.useState({
				...
				imgScale: currentBg && currentBg.size !== "cover" && currentBg.size !== "contain" ? Number(currentBg.size) || 100 : 100
			});

			const applyFromState = (kind, light, dark, desc, nextState, imagePath) => {
				const merged = { ...state, ...nextState };
				applyBg(kind, light, dark, desc, merged.strength, sizeOf(merged), theme, imagePath);
				setState((s) => ({ ...s, ...nextState, kind, rawLight: light, rawDark: dark, desc, busy: false, error: "" }));
			};

			const reset = () => {
				clearBg();
				saveBackground({ kind: "none" });
				...
				loadRemoteImage(p).then(
					(res) => {
						if (res && res.ok) {
							...
							applyFromState("image", value, value, "本地图片 " + p + "(" + String(res.bytes || 0) + " 字节)", {}, p);
						} else {
			...
			return React.createElement("div", { className: "bgst-root" },
				...
				React.createElement("div", null,
					React.createElement("div", { className: "bgst-label" }, "图片大小" + (state.kind !== "image" ? "(应用图片后可用)" : "")),
					React.createElement("div", { className: "bgst-row" },
						React.createElement("button", { type: "button", className: "bgst-btn" + (state.imgSize === "cover" ? " bgst-btn-active" : ""), disabled: state.kind !== "image", onClick: () => setImgSize("cover") }, "适应窗口"),
						React.createElement("button", { type: "button", className: "bgst-btn" + (state.imgSize === "contain" ? " bgst-btn-active" : ""), disabled: state.kind !== "image", onClick: () => setImgSize("contain") }, "完整显示"),
						React.createElement("button", { type: "button", className: "bgst-btn" + (state.imgSize === "scale" ? " bgst-btn-active" : ""), disabled: state.kind !== "image", onClick: () => setImgSize("scale") }, "自定义")),
					state.imgSize === "scale"
						? React.createElement("div", { className: "bgst-row" },
								React.createElement("input", { type: "range", min: "30", max: "200", step: "5", className: "bgst-range", value: state.imgScale, disabled: state.kind !== "image", onChange: (e) => setImgScale(Number(e.target.value)) }),
		...
		// Cordis service names: the kernel parks this plugin until the theme
		// and slots services are provided, then activates it.
		...
		function apply(ctx) {
			...
			// Restore the last background (persisted through the settings service);
			// DEFAULT_IMAGE is a fallback for first runs when it is configured.
			fetch("/dyn-bgimg/state").then((r) => r.json()).then(
				(res) => {
					if (res && res.ok && res.state && res.state.kind && res.state.kind !== "none") {
						applyBg(
							res.state.kind,
							res.state.light,
							res.state.dark,
							res.state.desc ?? "",
							typeof res.state.strength === "number" ? res.state.strength : 55,
							res.state.size === "contain" ? "contain" : res.state.size === "cover" ? "cover" : String(res.state.size),
							theme,
							res.state.imagePath
						);
						...
						loadRemoteImage(DEFAULT_IMAGE).then(
							(result) => {
								if (result && result.ok) {
									...
									applyBg("image", value, value, "自动加载:" + DEFAULT_IMAGE, 55, "contain", theme, DEFAULT_IMAGE);
								}
							},
							() => { /* best-effort */ }
						);
		...
		// Non-enumerable test hook: exposes the pure helpers to Node unit
		// tests without affecting the Cordis plugin contract (apply/inject).
		Object.defineProperty(exports, "__test", {
			value: { sizeOf, clampOpacity, resolveSizeCss, buildBackgroundCss },
			enumerable: false
		});
