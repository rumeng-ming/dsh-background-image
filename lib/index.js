...
import { MAX_BYTES, parseImagePath, normalizeSaveState, sanitizeSavedState, isPathInsideDir } from "./logic.js"

const SETTINGS_NAMESPACE = "dsh-background-image"

// Optional directory whitelist for local images (defense-in-depth).
// Empty (default) = any path is allowed, preserving the original behavior.
// Populate it to restrict POST /dyn-bgimg/load to specific directories:
//   const ALLOWED_DIRS = ["C:\\Users\\you\\Pictures"]
...
function apply(ctx) {
  ...
  async function loadImage(path) {
    ...
    if (ALLOWED_DIRS.length > 0 && !ALLOWED_DIRS.some((dir) => isPathInsideDir(target, dir))) {
      throw new Error("该路径不在允许的目录内")
    }
...
export { apply, inject, ALLOWED_DIRS }
